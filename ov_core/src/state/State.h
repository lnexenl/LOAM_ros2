...
class State {
 public:
  ...
  /**
   * @brief Gyroscope and accelerometer intrinsic matrix (scale imperfection and
   * axis misalignment)
   *
   ...
   *
   * This is used to construct our state transition which depends on if we are
   * estimating calibration. 15 if doing intrinsics, another +9 if doing grav
   * sensitivity
   *
  ...
  /// Lidar extrinsics
  std::shared_ptr<ov_type::PoseHM> _calib_IMUtoLid;

  /// Rotation from gyroscope frame to the "IMU" accelerometer frame (kalibr
  /// model)
  std::shared_ptr<ov_type::HMQuat> _calib_imu_GYROtoIMU;

  /// Rotation from accelerometer to the "IMU" gyroscope frame frame (rpng
  /// model)
  std::shared_ptr<ov_type::HMQuat> _calib_imu_ACCtoIMU;
 ...
 private:
  ...
  // clone" will actually correctly add it to the covariance
