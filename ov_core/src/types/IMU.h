 ...
 *
 * Contains PoseHM, Vec velocity, Vec gyro bias, and Vec accel bias.
...
public:
  ...
  /**
   * @brief Performs update operation using HMQuat update for orientation, then
  ...
  /// Pose type access
  std::shared_ptr<PoseHM> pose() { return _pose; }

  /// Quaternion type access
  std::shared_ptr<HMQuat> q() { return _pose->q(); }
