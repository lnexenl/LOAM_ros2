...
#ifndef OV_TYPE_TYPE_PoseHM_H
#define OV_TYPE_TYPE_PoseHM_H
 ...
 *
 * Internally we use a HMQuat quaternion representation for the orientation and
 * 3D Vec position. Please see HMQuat for details on its update procedure and
 * its left multiplicative error.
 */
class PoseHM : public Type {
public:
  PoseHM() : Type(6) {
    // Initialize subvariables
    _q = std::shared_ptr<HMQuat>(new HMQuat());
  ...
  /**
   * @brief Update q and p using a the HMQuat update for orientation and vector
   * update for position
   *
  ...
  std::shared_ptr<Type> clone() override {
    auto Clone = std::shared_ptr<PoseHM>(new PoseHM());
  ...
  std::shared_ptr<HMQuat> q() { return _q; }
